...
for i =1:size(c_aq_old,1) 
   ...
   while sum(c_a(i,:)) > 0 && c_b(i,:) > 0 
    ...
    if sum(fcfa.*c_a_tot(i,:)) < sum(Xi.*fcfb.*c_b_tot(i,:))
        % not sure if molar fraction is needed here since summing it 
    ...
    % reaction that occurs if napl compounds are greater than oxygen
     ...
     c_b_tot(i,:) = c_b(i,:) + sum((1./fcfb).*c_c(i,:));
  
   end
   
   % equilibration following reaction 
    [a,n_napl(i,1:24)] = equilibrate(c_a(i,1:24), n_napl_old(i,1:24), poros, Si, rho, MW, dx, A);
    c_aq(i,1:24) = a;
    c_aq(i,25) = c_b(i,:);
